...
out nothing. A public probe that found the door, tried the handle wrong, and left. A full scan of all
16,940 `withdraw` calls in the contract's history shows only three accounts ever passed a negative
amount: that June probe, and the two September exploiters.
...
- **Watch the probes.** The June `gatedepostf` attempt was visible on-chain for three months. Monitoring for
  negative or malformed amounts on ledger contracts is cheap.
